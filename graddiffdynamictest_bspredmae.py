  ...
  while len(total_set - eval_set) > 0 or len(available_sample) > 0:
    ...
    if len(available_sample) > 0:
      ...
      for j in range(num_cpus):
        p = Process(target=get_costs,args=(synth_reg,train_samples,train_sample_costs,eval_features,eval_labels,profile_features,labels,cur_X_train,cur_y_train,num_to_profile,available_list,j*num_work,min(num_work,total_work),K))
        p.start()
        sub_procs.append(p)
      ...
      train_best_sample_idx = np.argmin(train_sample_costs)
    ...
    if len(eval_available_list) > 0:
      ...
      for j in range(len(eval_available_list)):
        ...
        new_X_train = np.array([profile_features[idx] for idx in new_sample_idx])
        new_y_train = np.array([labels[idx] for idx in new_sample_idx])
        eval_samples.append((new_X_train,new_y_train,new_sample_idx)) 

      eval_sample_costs = Manager().list([0 for j in range(len(eval_available_list))])
      sub_procs = []
      total_work = len(eval_available_list)
      num_cpus = 32
      num_work = int(np.ceil(total_work/num_cpus))
      for j in range(num_cpus):
        p = Process(target=get_eval_costs,args=(cur_reg,synth_reg,eval_samples,eval_sample_costs,eval_features,eval_labels,profile_features,labels,eval_available_list,j*num_work,min(num_work,total_work),K))
        p.start()
        sub_procs.append(p)
        total_work = max(0,total_work - num_work)
      for j in range(num_cpus):
