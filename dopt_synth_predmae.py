...
def get_costs(synth_reg,samples,sample_costs,eval_features,eval_labels,profile_features,labels,cur_X_train,cur_y_train,num_to_profile,available_list,start_index,num_work,K):
  for j in range(start_index,start_index+num_work):
    ...
    reg = RandomForestRegressor(n_estimators=100).fit(np.concatenate((cur_X_train,new_X_train)),np.concatenate((cur_y_train,synth_reg.predict(new_X_train))))
...
json.dump(avg_results.tolist(),open("avg_doptsynth10_predmae_10sim.json","w"))
json.dump(min_results.tolist(),open("min_doptsynth10_predmae_10sim.json","w"))
json.dump(max_results.tolist(),open("max_doptsynth10_predmae_10sim.json","w"))

json.dump(avg_val_results.tolist(),open("avg_val_doptsynth10_predmae_10sim.json","w"))
json.dump(min_val_results.tolist(),open("min_val_doptsynth10_predmae_10sim.json","w"))
json.dump(max_val_results.tolist(),open("max_val_doptsynth10_predmae_10sim.json","w"))


json.dump(this_train_sizes.tolist(),open("trainsize_doptsynth10_predmae_10sim.json","w"))
